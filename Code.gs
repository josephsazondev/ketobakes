...
function doPost(e) {
  try {
    ...
    else if (d.type === 'update') updateEntry(d.rowId, d);
...
// Overwrites an existing row, located by rowId (e.g. 'I_5' / 'E_3').
// The sheet is derived from the rowId prefix (I_ = income, E_ = expense) —
// this is authoritative and caller-independent, so every write path (toggle
// paid, full edit) works without relying on an extra payload field.
function updateEntry(rowId, d) {
  if (!rowId) throw new Error('Missing rowId');
  var parts = String(rowId).split('_');
  var key = parts[0] === 'I' ? 'income' : parts[0] === 'E' ? 'expense' : null;
  if (!key) throw new Error('Bad rowId (expected I_/E_ prefix): ' + rowId);
  ...
  var values = {
    ...
    amount: Number(d.amount) || 0,
    payer: d.payer || '',
    paid: Boolean(d.paid),  // Explicitly coerce to boolean
    description: d.description || '',
  };
  Object.keys(map).forEach(function (f) { row[map[f]] = values[f]; });
